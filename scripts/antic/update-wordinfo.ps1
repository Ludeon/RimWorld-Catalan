...
# Create files
foreach ($fileName in "Male", "Female", "Neuter", "Other", "new_words", "male_add", "female_add")
{
...
# Merge found male words into the list of male words and also the "male_add" persistent list
Get-Content "$temp/all_males.txt", "$main/male_add.txt", "$main/Male.txt" | Sort-Object -Unique | Set-Content "$main/Male.txt"

# Merge found female words into the list of female words and also the "female_add" persistent list
Get-Content "$temp/all_females.txt", "$main/Female.txt", "$main/female_add.txt" | Sort-Object -Unique | Set-Content "$main/Female.txt"

# Save a list of all found words
Get-Content "$temp/all*.txt", "$main/male_add.txt", "$main/female_add.txt" | Sort-Object -Unique | Set-Content "$temp/all.txt"
